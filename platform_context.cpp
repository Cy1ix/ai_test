...
namespace frame {
    namespace platform {
        ...
        std::string getTempPathFromEnvironment() {
        ...
        std::vector<std::string> getCommandLineArgs() {
            ...
            for (int i = 1; i < argc; i++) {
                args.push_back(wstrToStr(argv[i]));
            }
        ...
        WindowsPlatformContext::WindowsPlatformContext(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR lpCmdLine, INT nCmdShow) :
            ...
            m_external_storage_directory = "";
            m_temp_directory = getTempPathFromEnvironment();
            m_arguments = getCommandLineArgs();
