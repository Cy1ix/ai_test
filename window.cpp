/* Copyright (c) 2018-2023, Arm Limited and Contributors
 *
 * SPDX-License-Identifier: Apache-2.0
 *
 * Licensed under the Apache License, Version 2.0 the "License";
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "platform/window.h"
#include "platform/platform.h"

#include "GLFW/glfw3.h"

namespace frame {
	namespace platform {
        namespace {
            void errorCallback(int error, const char* description) {
                LOGE("GLFW Error (code {}): {}", error, description);
            }

            void windowCloseCallback(GLFWwindow* window) {
                glfwSetWindowShouldClose(window, GLFW_TRUE);
            }

            void windowSizeCallback(GLFWwindow* window, int width, int height) {
                if (auto platform = reinterpret_cast<Platform*>(glfwGetWindowUserPointer(window))) {
                    platform->resize(width, height);
                }
            }

            void windowFocusCallback(GLFWwindow* window, int focused) {
                if (auto platform = reinterpret_cast<Platform*>(glfwGetWindowUserPointer(window))) {
                    platform->setFocus(focused);
                }
            }

            inline KeyCode translateKeyCode(int key) {
                static const std::unordered_map<int, KeyCode> key_lookup = {
                    {GLFW_KEY_SPACE, KeyCode::Space},
                    {GLFW_KEY_APOSTROPHE, KeyCode::Apostrophe},
                    {GLFW_KEY_COMMA, KeyCode::Comma},
                    {GLFW_KEY_MINUS, KeyCode::Minus},
                    {GLFW_KEY_PERIOD, KeyCode::Period},
                    {GLFW_KEY_SLASH, KeyCode::Slash},
                    {GLFW_KEY_0, KeyCode::_0},
                    {GLFW_KEY_1, KeyCode::_1},
                    {GLFW_KEY_2, KeyCode::_2},
                    {GLFW_KEY_3, KeyCode::_3},
                    {GLFW_KEY_4, KeyCode::_4},
                    {GLFW_KEY_5, KeyCode::_5},
                    {GLFW_KEY_6, KeyCode::_6},
                    {GLFW_KEY_7, KeyCode::_7},
                    {GLFW_KEY_8, KeyCode::_8},
                    {GLFW_KEY_9, KeyCode::_9},
                    {GLFW_KEY_SEMICOLON, KeyCode::Semicolon},
                    {GLFW_KEY_EQUAL, KeyCode::Equal},
                    {GLFW_KEY_A, KeyCode::A},
                    {GLFW_KEY_B, KeyCode::B},
                    {GLFW_KEY_C, KeyCode::C},
                    {GLFW_KEY_D, KeyCode::D},
                    {GLFW_KEY_E, KeyCode::E},
                    {GLFW_KEY_F, KeyCode::F},
                    {GLFW_KEY_G, KeyCode::G},
                    {GLFW_KEY_H, KeyCode::H},
                    {GLFW_KEY_I, KeyCode::I},
                    {GLFW_KEY_J, KeyCode::J},
                    {GLFW_KEY_K, KeyCode::K},
                    {GLFW_KEY_L, KeyCode::L},
                    {GLFW_KEY_M, KeyCode::M},
                    {GLFW_KEY_N, KeyCode::N},
                    {GLFW_KEY_O, KeyCode::O},
                    {GLFW_KEY_P, KeyCode::P},
                    {GLFW_KEY_Q, KeyCode::Q},
                    {GLFW_KEY_R, KeyCode::R},
                    {GLFW_KEY_S, KeyCode::S},
                    {GLFW_KEY_T, KeyCode::T},
                    {GLFW_KEY_U, KeyCode::U},
                    {GLFW_KEY_V, KeyCode::V},
                    {GLFW_KEY_W, KeyCode::W},
                    {GLFW_KEY_X, KeyCode::X},
                    {GLFW_KEY_Y, KeyCode::Y},
                    {GLFW_KEY_Z, KeyCode::Z},
                    {GLFW_KEY_LEFT_BRACKET, KeyCode::LeftBracket},
                    {GLFW_KEY_BACKSLASH, KeyCode::Backslash},
                    {GLFW_KEY_RIGHT_BRACKET, KeyCode::RightBracket},
                    {GLFW_KEY_GRAVE_ACCENT, KeyCode::GraveAccent},
                    {GLFW_KEY_ESCAPE, KeyCode::Escape},
                    {GLFW_KEY_ENTER, KeyCode::Enter},
                    {GLFW_KEY_TAB, KeyCode::Tab},
                    {GLFW_KEY_BACKSPACE, KeyCode::Backspace},
                    {GLFW_KEY_INSERT, KeyCode::Insert},
                    {GLFW_KEY_DELETE, KeyCode::DelKey},
                    {GLFW_KEY_RIGHT, KeyCode::Right},
                    {GLFW_KEY_LEFT, KeyCode::Left},
                    {GLFW_KEY_DOWN, KeyCode::Down},
                    {GLFW_KEY_UP, KeyCode::Up},
                    {GLFW_KEY_PAGE_UP, KeyCode::PageUp},
                    {GLFW_KEY_PAGE_DOWN, KeyCode::PageDown},
                    {GLFW_KEY_HOME, KeyCode::Home},
                    {GLFW_KEY_END, KeyCode::End},
                    {GLFW_KEY_CAPS_LOCK, KeyCode::CapsLock},
                    {GLFW_KEY_SCROLL_LOCK, KeyCode::ScrollLock},
                    {GLFW_KEY_NUM_LOCK, KeyCode::NumLock},
                    {GLFW_KEY_PRINT_SCREEN, KeyCode::PrintScreen},
                    {GLFW_KEY_PAUSE, KeyCode::Pause},
                    {GLFW_KEY_F1, KeyCode::F1},
                    {GLFW_KEY_F2, KeyCode::F2},
                    {GLFW_KEY_F3, KeyCode::F3},
                    {GLFW_KEY_F4, KeyCode::F4},
                    {GLFW_KEY_F5, KeyCode::F5},
                    {GLFW_KEY_F6, KeyCode::F6},
                    {GLFW_KEY_F7, KeyCode::F7},
                    {GLFW_KEY_F8, KeyCode::F8},
                    {GLFW_KEY_F9, KeyCode::F9},
                    {GLFW_KEY_F10, KeyCode::F10},
                    {GLFW_KEY_F11, KeyCode::F11},
                    {GLFW_KEY_F12, KeyCode::F12},
                    {GLFW_KEY_KP_0, KeyCode::KP_0},
                    {GLFW_KEY_KP_1, KeyCode::KP_1},
                    {GLFW_KEY_KP_2, KeyCode::KP_2},
                    {GLFW_KEY_KP_3, KeyCode::KP_3},
                    {GLFW_KEY_KP_4, KeyCode::KP_4},
                    {GLFW_KEY_KP_5, KeyCode::KP_5},
                    {GLFW_KEY_KP_6, KeyCode::KP_6},
                    {GLFW_KEY_KP_7, KeyCode::KP_7},
                    {GLFW_KEY_KP_8, KeyCode::KP_8},
                    {GLFW_KEY_KP_9, KeyCode::KP_9},
                    {GLFW_KEY_KP_DECIMAL, KeyCode::KP_Decimal},
                    {GLFW_KEY_KP_DIVIDE, KeyCode::KP_Divide},
                    {GLFW_KEY_KP_MULTIPLY, KeyCode::KP_Multiply},
                    {GLFW_KEY_KP_SUBTRACT, KeyCode::KP_Subtract},
                    {GLFW_KEY_KP_ADD, KeyCode::KP_Add},
                    {GLFW_KEY_KP_ENTER, KeyCode::KP_Enter},
                    {GLFW_KEY_KP_EQUAL, KeyCode::KP_Equal},
                    {GLFW_KEY_LEFT_SHIFT, KeyCode::LeftShift},
                    {GLFW_KEY_LEFT_CONTROL, KeyCode::LeftControl},
                    {GLFW_KEY_LEFT_ALT, KeyCode::LeftAlt},
                    {GLFW_KEY_RIGHT_SHIFT, KeyCode::RightShift},
                    {GLFW_KEY_RIGHT_CONTROL, KeyCode::RightControl},
                    {GLFW_KEY_RIGHT_ALT, KeyCode::RightAlt},
                };

                auto key_it = key_lookup.find(key);
                return key_it != key_lookup.end() ? key_it->second : KeyCode::Unknown;
            }

            inline KeyAction translateKeyAction(int action) {
                switch (action) {
                case GLFW_PRESS:   return KeyAction::Down;
                case GLFW_RELEASE: return KeyAction::Up;
                case GLFW_REPEAT:  return KeyAction::Repeat;
                default:           return KeyAction::Unknown;
                }
            }

            void keyCallback(GLFWwindow* window, int key, int /*scancode*/, int action, int /*mods*/) {
                KeyCode key_code = translateKeyCode(key);
                KeyAction key_action = translateKeyAction(action);

                if (auto platform = reinterpret_cast<Platform*>(glfwGetWindowUserPointer(window))) {
                    platform->inputEvent(KeyInputEvent{ key_code, key_action });
                }
            }

            inline MouseButton translateMouseButton(int button) {
                return (button < GLFW_MOUSE_BUTTON_6) ?
                    static_cast<MouseButton>(button) : MouseButton::Unknown;
            }

            inline MouseAction translateMouseAction(int action) {
                return (action == GLFW_PRESS) ? MouseAction::Down :
                    (action == GLFW_RELEASE) ? MouseAction::Up : MouseAction::Unknown;
            }

            void cursorPositionCallback(GLFWwindow* window, double xpos, double ypos) {
                if (auto platform = reinterpret_cast<Platform*>(glfwGetWindowUserPointer(window))) {
                    platform->inputEvent(MouseButtonInputEvent{
                        MouseButton::Unknown,
                        MouseAction::Move,
                        static_cast<float>(xpos),
                        static_cast<float>(ypos)
                        });
                }
            }

            void mouseButtonCallback(GLFWwindow* window, int button, int action, int /*mods*/) {
                if (auto platform = reinterpret_cast<Platform*>(glfwGetWindowUserPointer(window))) {
                    double xpos, ypos;
                    glfwGetCursorPos(window, &xpos, &ypos);

                    platform->inputEvent(MouseButtonInputEvent{
                        translateMouseButton(button),
                        translateMouseAction(action),
                        static_cast<float>(xpos),
                        static_cast<float>(ypos)
                        });
                }
            }
        }

        std::unique_ptr<Window> Window::Create(Platform* platform, const Properties& properties) {
            auto window = std::unique_ptr<Window>(new Window(platform, properties));

            bool success = false;
            switch (properties.backend) {
            case Backend::GLFW:
                success = window->initGLFW();
                break;
            default:
                LOGE("Unsupported window backend");
                break;
            }

            if (!success) {
                return nullptr;
            }

            return window;
        }

        Window::Window(Platform* platform, const Properties& properties) :
            m_platform(platform),
            m_properties(properties)
        {
        }

        Window::~Window() {
            if (m_glfw_handle) {
                glfwDestroyWindow(m_glfw_handle);
                glfwTerminate();
            }
        }

        bool Window::initGLFW() {
#if defined(VK_USE_PLATFORM_XLIB_KHR)
            glfwInitHint(GLFW_X11_XCB_VULKAN_SURFACE, false);
#endif

            if (!glfwInit()) {
                LOGE("Failed to initialize GLFW");
                return false;
            }

            glfwSetErrorCallback(errorCallback);
            glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

            switch (m_properties.mode) {
            case Mode::Fullscreen: {
                auto* monitor = glfwGetPrimaryMonitor();
                const auto* mode = glfwGetVideoMode(monitor);
                m_glfw_handle = glfwCreateWindow(mode->width, mode->height, m_properties.title.c_str(), monitor, nullptr);
                break;
            }

            case Mode::FullscreenBorderless: {
                auto* monitor = glfwGetPrimaryMonitor();
                const auto* mode = glfwGetVideoMode(monitor);
                glfwWindowHint(GLFW_RED_BITS, mode->redBits);
                glfwWindowHint(GLFW_GREEN_BITS, mode->greenBits);
                glfwWindowHint(GLFW_BLUE_BITS, mode->blueBits);
                glfwWindowHint(GLFW_REFRESH_RATE, mode->refreshRate);
                m_glfw_handle = glfwCreateWindow(mode->width, mode->height, m_properties.title.c_str(), monitor, nullptr);
                break;
            }

            default:
                m_glfw_handle = glfwCreateWindow(
                    static_cast<int>(m_properties.extent.width),
                    static_cast<int>(m_properties.extent.height),
                    m_properties.title.c_str(), nullptr, nullptr);
                break;
            }

            if (!m_glfw_handle) {
                LOGE("Failed to create GLFW window");
                glfwTerminate();
                return false;
            }
            
            m_properties.extent = resize(m_properties.extent);
            
            glfwSetWindowUserPointer(m_glfw_handle, m_platform);
            glfwSetWindowCloseCallback(m_glfw_handle, windowCloseCallback);
            glfwSetWindowSizeCallback(m_glfw_handle, windowSizeCallback);
            glfwSetWindowFocusCallback(m_glfw_handle, windowFocusCallback);
            glfwSetKeyCallback(m_glfw_handle, keyCallback);
            glfwSetCursorPosCallback(m_glfw_handle, cursorPositionCallback);
            glfwSetMouseButtonCallback(m_glfw_handle, mouseButtonCallback);

            glfwSetInputMode(m_glfw_handle, GLFW_STICKY_KEYS, 1);
            glfwSetInputMode(m_glfw_handle, GLFW_STICKY_MOUSE_BUTTONS, 1);

            return true;
        }

        vk::SurfaceKHR Window::createSurface(core::Instance& instance) {
            if (instance.getHandle() == vk::Instance{} || !m_glfw_handle) {
                return {};
            }

            VkSurfaceKHR surface;
            
            if (glfwCreateWindowSurface(instance.getHandle(), m_glfw_handle, NULL, &surface) != VK_SUCCESS) {
                throw std::runtime_error("Create window surface fail");
            }

            return vk::SurfaceKHR{ surface };
        }
        
        bool Window::shouldClose() const {
            if (m_properties.backend == Backend::GLFW) {
                return m_glfw_handle ? glfwWindowShouldClose(m_glfw_handle) : true;
            }
            return true;
        }

        void Window::processEvents() {
            if (m_properties.backend == Backend::GLFW) {
                glfwPollEvents();
            }
        }

        void Window::close() {
            if (m_properties.backend == Backend::GLFW && m_glfw_handle) {
                glfwSetWindowShouldClose(m_glfw_handle, GLFW_TRUE);
            }
        }

        float Window::getDpiFactor() const {
            if (m_properties.backend == Backend::GLFW) {
                auto primary_monitor = glfwGetPrimaryMonitor();
                auto vidmode = glfwGetVideoMode(primary_monitor);

                int width_mm, height_mm;
                glfwGetMonitorPhysicalSize(primary_monitor, &width_mm, &height_mm);

                static constexpr float inch_to_mm = 25.0f;
                static constexpr float win_base_density = 96.0f;

                auto dpi = static_cast<uint32_t>(static_cast<float>(vidmode->width) / (static_cast<float>(width_mm) / inch_to_mm));
                return static_cast<float>(dpi) / win_base_density;
            }
            return 1.0f;
        }

        float Window::getContentScaleFactor() const {
            if (m_properties.backend == Backend::GLFW && m_glfw_handle) {
                int fb_width, fb_height;
                glfwGetFramebufferSize(m_glfw_handle, &fb_width, &fb_height);

                int win_width, win_height;
                glfwGetWindowSize(m_glfw_handle, &win_width, &win_height);

                return static_cast<float>(fb_width) / static_cast<float>(win_width);
            }
            return 1.0f;
        }

        Window::Extent Window::resize(const Extent& new_extent) {
            if (m_properties.resizable) {
                m_properties.extent = new_extent;
            }
            return m_properties.extent;
        }

        bool Window::getDisplayPresentInfo(VkDisplayPresentInfoKHR* info, uint32_t src_width, uint32_t src_height) const {
            // Not implemented for standard window backends
            return false;
        }

        std::vector<const char*> Window::getRequiredSurfaceExtensions() const {
            if (m_properties.backend == Backend::GLFW) {
                uint32_t glfw_extension_count{ 0 };
                const char** names = glfwGetRequiredInstanceExtensions(&glfw_extension_count);
                return { names, names + glfw_extension_count };
            }
            return {};
        }
	}
}
